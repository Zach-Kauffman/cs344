...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		if(pid == 0) { //child process
			...
			if(bigBuffer[0] != 'E') {
				fprintf(stderr, "ERR: CLIENT NOT OTP_ENC\n");
				...
				char* final = encrypt(plaintext, key);
				//send encrypted string back in chunks
...
char* encrypt(char* toEnc, char* key) {
	int ii;
	int enc;
	char* buffer = malloc(strlen(toEnc)); //string to return is same length as string to be encrypted
	for(ii = 0; ii < strlen(toEnc); ii ++) {
		//handle spaces in key or text by converting to the character after 'Z'
		if(toEnc[ii] == 32) {
			toEnc[ii] = 91;
		}
		if(key[ii] == 32) {
		...
		//handle endlines by just skipping them
		if(toEnc[ii] != '\n') {
			enc = (((toEnc[ii] + key[ii]) - 130) % 27); //add letters together, get value 0-26
			//if the char gets converted to what should be a space, turn it back into a space
			if(enc == 26) {
				enc = 32;
			//if the char gets converted to what should be \n, convert back
			} else {
				enc += 65;
			}	
		}
