...
int main(int argc, char *argv[])
{
	...
	//check length
	dec = open(argv[1], O_RDONLY);
	decLen = lseek(dec, 0, SEEK_END);
	key = open(argv[2], O_RDONLY);
	keyLen = lseek(key, 0, SEEK_END);
	
	plainText = malloc(decLen);
	...
	//build a data string
	//begins with 'D$' to verify decryption
	//Puts '$' after plaintext and key
	//Puts '!' at the end
	data = malloc(decLen + keyLen + 20);
	strcpy(data, "D$");
	plainText = dumpText(argv[1], decLen);
	...
	//server returns '&' if connection is rejected, otherwise returns decrypted string
	//if it's a good connection, server appends '$' to the end of the string
	finalText = malloc(decLen);
	strcpy(finalText, "");
	while(1){
		...
		if(c > 0) {
			...
			if(finalText[0] == '&') { //check for good connection
				fprintf(stderr, "ERR: DEC CLIENT AUTH ERR\n");
				exit(2);
			}
